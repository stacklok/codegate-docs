...
4. Remove the CodeGate CA certificate from your system's trust store.
   ...
   - **Linux:**
     ...
     ```bash
     # Ubuntu/Debian:
     sudo rm /usr/local/share/ca-certificates/codegate.crt && sudo update-ca-certificates

     # RHEL/Fedora:
     sudo rm /etc/pki/ca-trust/source/anchors/codegate.pem && sudo update-ca-trust
     ```
