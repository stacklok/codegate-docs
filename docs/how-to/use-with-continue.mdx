...
CodeGate works with the following AI model providers via Continue:
...
1. Add the `apiBase` property to the `models` entry (chat) and
   ...
   Replace `/<provider>/` with `/anthropic/`, `/ollama/`, `/openai/`, or
   `/vllm/`.
...
provider. For each, replace the values in ALL_CAPS. The configuration syntax is
the same for VS Code and JetBrains IDEs.
...
1. If you launched CodeGate with a persistent volume, delete the local directory
   you mapped to remove the CodeGate database and other files.
