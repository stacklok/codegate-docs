---
title: Install and manage ToolHive
...
If you use Homebrew on macOS, this is the simplest installation method:
...
available. When you run a ToolHive command, it will display a message if an
update exists.
...
If you installed ToolHive via Homebrew, you can upgrade it with:
...
If you built ToolHive from source, you can upgrade it by pulling the latest
...
For detailed documentation on each command, refer to the
...
If you need to uninstall ToolHive:
...
{/* prettier-ignore */}
   <Tabs groupId='installer' queryString='installer'>
     <TabItem value='homebrew' label='Homebrew (macOS)' default>
       If you installed ToolHive via Homebrew, you can uninstall it with:
...
2. If using Docker on Linux, ensure your user has permission to access the
   Docker socket:
...
- Quickstart: [Getting started with ToolHive](../quickstart.mdx)
