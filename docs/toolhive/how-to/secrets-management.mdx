...
allowing you to manage these values securely without exposing them in plaintext
configuration files.
...
`thv secret` command for the first time, ToolHive prompts you for a password to
encrypt and decrypt your secrets.
...
need to enter it every time you use a `thv secret` command.
...
`thv secret get` command and the `--secret` parameter for `thv run`. You need to
...
creation process. This token is required for all `thv secret` commands.
...
The [`thv secret set`](../reference/cli/thv_secret_set.md) command allows you to
...
Alternately, you can set a secret using standard input:
...
ToolHive can securely pass secrets to an MCP server when you run it. This allows
the server to access sensitive information without exposing it in plaintext.

To do this, use the `--secret` flag with the `thv run` command. The secret value
...
Review the MCP server's documentation to find the expected environment variable
name(s). For example, the GitHub MCP server expects the GitHub token to be
passed as `GITHUB_PERSONAL_ACCESS_TOKEN`.
...
3. On some Linux systems, you may need to install additional packages:
...
3. Ensure the secret reference URI is correct and matches the vault, item, and
   field names in 1Password.
