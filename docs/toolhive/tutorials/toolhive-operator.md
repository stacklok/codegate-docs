...
- How to manage MCP servers using kubectl

## What you'll build

By the end of this tutorial, you'll have:

- A local Kubernetes cluster running the ToolHive operator
- A working MCP server (the fetch server) deployed as a Kubernetes resource
- Understanding of how to manage MCP servers using Kubernetes resources
...
If you want to get up and running quickly, you can use our automated setup.
...
If you prefer to understand each step or don't have Task and Helm installed,
continue with the manual setup below.
...
Now deploy the ToolHive operator to your cluster. The operator will watch for
MCP server resources and manage their lifecycle automatically.

First, install the Custom Resource Definition (CRD) that defines what an MCP
server looks like to Kubernetes:

```bash
kubectl apply --server-side -f https://raw.githubusercontent.com/stacklok/toolhive/main/deploy/operator/crds/toolhive.stacklok.dev_mcpservers.yaml
```
...
Set up the necessary permissions (RBAC):
...
You'll deploy the fetch server, which allows AI assistants to retrieve web
content.

Apply the sample MCP server from the ToolHive repository:
...
(like AI assistants) can use to communicate with your MCP server.

:::

## Step 5: Connect your AI assistant to the MCP server

Now that your MCP server is running in Kubernetes, connect it to an AI
assistant. You'll use Visual Studio Code with GitHub Copilot as an example.
...
Save the file and click "Start" to initiate the connection. The indicator should
change to "Running" and show "1 tools".
...
- Learn about [advanced MCP server configurations](../kubernetes/run-mcp-k8s.md)
  for production deployments
- Explore
  [deploying the operator with Helm](../kubernetes/deploy-operator-helm.md) for
  easier management
- Set up [production deployments](../kubernetes/deploy-operator-kubectl.md) with
  proper security and scaling
