...
In this tutorial, you'll learn how to install ToolHive and run your first MCP
server. By the end, you'll have a working MCP server that can be used by AI
assistants to fetch content from websites.
...
- How to use the server with an AI assistant
...
- A supported client (VS Code with Copilot, Cursor, or Roo Code)

## Step 1: Install ToolHive

First, let's install ToolHive on your system. For detailed installation
instructions, see the [Installing ToolHive](../how-to/install.mdx) guide.
...
You should see the current version of ToolHive displayed.
...
You should see a confirmation message that auto-discovery has been enabled.

:::info What's happening?

When auto-discovery is enabled, ToolHive will automatically find and configure
[supported clients](../reference/client-compatibility.md) on your system. This
...
`thv config register-client --help` to learn how to explicitly register a
client.
...
Let's see what MCP servers are available in the registry:

```bash
thv registry list
```

You'll see a list of available servers with their names and descriptions.
...
For this tutorial, we'll use the "fetch" server, which is a simple MCP server
that allows AI assistants to get the contents of a website. To learn more about
the server before running it, run:

```bash
thv registry info fetch
```

This will show you detailed information about the server, including what tools
it provides and any configuration options.

## Step 4: Run the Fetch MCP server

Now, let's run the Fetch server:
...
and starts the server. It also sets up a proxy that allows your AI assistant to
...
ToolHive keeps track of all the MCP servers it's managing. The `list` command
shows you which servers are running and how they're configured.

:::

## Step 6: Use the MCP server with your AI assistant

Now that your MCP server is running, you can use it with your AI assistant. Open
your supported client (VS Code, Cursor, etc.) and ask the AI to fetch content
from a website. Note that you might need to restart your client for the changes
to take effect.
...
`thv restart` or `thv run`. Removing a server completely deletes the container,
...
### AI assistant can't use the server

If your AI assistant can't use the server:
...
- Try running other MCP servers from the registry with `thv registry list` and
  `thv run <server-name>`
- Learn about [secrets management](../how-to/secrets-management.mdx) for MCP
  servers that require authentication
- Explore [custom permissions](../how-to/custom-permissions.md) for MCP servers
- Set up [shell completion](../how-to/install.mdx#enable-shell-completion) to
  make ToolHive commands easier to use
- Learn how to [upgrade ToolHive](../how-to/install.mdx#upgrade-toolhive) when
